...
void Timer_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;//上拉输入。虽然手册建议给TIM2的ETR引脚配置为浮空输入
	                                             //如果外部输入信号功率很小，避免上拉电阻的影响则浮空输入
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//50MHz
	GPIO_Init(GPIOA, &GPIO_InitStructure);
    //2.选择时基单元的时钟源
    TIM_ETRClockMode2Config(TIM2, TIM_ExtTRGPSC_OFF, 
                             TIM_ExtTRGPolarity_NonInverted, 0x00);//将TIM2的时钟源设置为通过ETR的外部时钟2
    //3.配置时基单元
    ...
    TIM_TimeBaseInitStructure.TIM_Period = 10 - 1;//自动重装载值。计数器计数到这个值后会申请中断，产生更新事件
